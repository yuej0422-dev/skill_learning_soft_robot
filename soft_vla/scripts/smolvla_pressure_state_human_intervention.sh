...
#
# 推荐测试顺序：
#   1) mock：DEVICE=cpu DURATION_S=2 WAIT_FOR_START_KEY=0 bash soft_vla/scripts/smolvla_pressure_state_human_intervention.sh
#   2) 实时观测但不下发压力：STATE_HARDWARE=1 VLA_BACKEND=smolvla LIVE_OBSERVATION=1 CAMERA_PREVIEW=1 bash soft_vla/scripts/smolvla_pressure_state_human_intervention.sh
#   3) 实物：RESET_INTEGRAL_ON_TARGET=1 RUN_HARDWARE=1 VLA_BACKEND=smolvla LIVE_OBSERVATION=1 CAMERA_PREVIEW=1 bash soft_vla/scripts/smolvla_pressure_state_human_intervention.sh
...
args=(
  soft_vla/scripts/deploy_smolvla_pressure_state_human_intervention.py
  --mode "$MODE"
