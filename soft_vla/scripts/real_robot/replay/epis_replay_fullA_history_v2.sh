...
#   HARDWARE_ENABLED=0 MAX_FRAMES=2 bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
# 上层每1秒发送一个target、总共发送3次：
#   TARGET_FREQUENCY=1 MAX_FRAMES=3 bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
# 每次切换target前清空积分误差q（默认0，保持积分连续）：
#   RESET_INTEGRAL_ON_TARGET=1 TARGET_FREQUENCY=10 MAX_FRAMES=3 bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
# 上层每10秒发送一个target、总共发送2次：
#   TARGET_FREQUENCY=0.1 MAX_FRAMES=2 bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
# 使用10Hz Full-A Koopman时，需同时覆盖checkpoint与底层频率：
#   KOOPMAN_CHECKPOINT=/path/to/10hz/best.pt FREQUENCY=10 TARGET_FREQUENCY=1 MAX_FRAMES=3 bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
# 实物小段验证：
#   MAX_FRAMES=2 PRESSURE_SCALE=0.2 bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
# 离线生成临时固定 K 后回放：
#   TARGET_FREQUENCY=10 MAX_FRAMES=0 KOOPMAN_CHECKPOINT=/home/cao/skill_learning_soft_robot/motion_control_training/koopman/experiments/fullA_history_v2/runs/robot_records_7_03_1_delta_tcp_fullA_history_v2_10hz_k50_hist10_epoch3000_wandb_online_20260712_2313/best.pt FREQUENCY=10 FEEDBACK=fixed_k_integral bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
#   TARGET_FREQUENCY=0.1 MAX_FRAMES=2 FREQUENCY=50 FEEDBACK=fixed_k_integral bash soft_vla/scripts/real_robot/replay/epis_replay_fullA_history_v2.sh
