...
# 常用方式：
#   1) 默认回放：bash soft_vla/scripts/real_robot/replay/epis_replay.sh
#   2) 换前馈：FEEDFORWARD=awac bash soft_vla/scripts/real_robot/replay/epis_replay.sh
#   3) 离线临时生成固定 K：FEEDBACK=fixed_k_integral bash soft_vla/scripts/real_robot/replay/epis_replay.sh
#   4) 调 Q/R 后生成固定 K：Q_TCP6_WEIGHT=2 R_WEIGHT=50 FEEDBACK=fixed_k_integral bash soft_vla/scripts/real_robot/replay/epis_replay.sh
#   5) 改输出目录名：RUN_LABEL=Qlearning FEEDFORWARD=awac bash soft_vla/scripts/real_robot/replay/epis_replay.sh
#   6) 上层1Hz、发送3次target：TARGET_FREQUENCY=1 MAX_FRAMES=3 bash soft_vla/scripts/real_robot/replay/epis_replay.sh
#   7) 上层0.1Hz、发送2次target：TARGET_FREQUENCY=0.1 MAX_FRAMES=20 FEEDBACK=fixed_k_integral bash soft_vla/scripts/real_robot/replay/epis_replay.sh
