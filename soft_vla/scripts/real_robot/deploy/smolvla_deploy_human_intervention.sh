...
# 原始 soft_vla/scripts/real_robot/deploy/smolvla_deploy.sh 不受影响；本脚本走新增 runtime。
#
# 推荐测试顺序：
#   1) 只测手柄：
...
#   3) 完整实物控制：
#      REMOTE_CONTROL_DEBUG=1 SAVE_HUMAN_EPISODES=1 RUN_HARDWARE=1 VLA_BACKEND=smolvla LIVE_OBSERVATION=1 CAMERA_PREVIEW=1   bash soft_vla/scripts/real_robot/deploy/smolvla_deploy_human_intervention.sh
#      REMOTE_CONTROL_DEBUG=0 SAVE_HUMAN_EPISODES=1 RUN_HARDWARE=1 VLA_BACKEND=smolvla LIVE_OBSERVATION=1 CAMERA_PREVIEW=1   bash soft_vla/scripts/real_robot/deploy/smolvla_deploy_human_intervention.sh
...
OUTPUT_DIR=${OUTPUT_DIR:-"$ROOT/soft_vla/artifacts/real_robot/$RUN_LABEL"}  # 本次运行所有输出根目录
...
args=(
  soft_vla/scripts/real_robot/components/deploy_smolvla_human_intervention.py
  --mode "$MODE"
