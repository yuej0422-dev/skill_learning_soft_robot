...
# SmolVLA 四进程部署入口。
# 常用方式：
#   1) 纯 mock：bash soft_vla/scripts/real_robot/deploy/smolvla_deploy.sh
#   2) 真实模型 + replay 观测 + mock IO：VLA_BACKEND=smolvla bash soft_vla/scripts/real_robot/deploy/smolvla_deploy.sh
#   3) 真实模型 + 三相机 + 动捕 + mock 压力：STATE_HARDWARE=1 VLA_BACKEND=smolvla LIVE_OBSERVATION=1 CAMERA_PREVIEW=1 bash soft_vla/scripts/real_robot/deploy/smolvla_deploy.sh
#   4) 完整实物控制：RUN_HARDWARE=1 VLA_BACKEND=smolvla LIVE_OBSERVATION=1 CAMERA_PREVIEW=1 bash soft_vla/scripts/real_robot/deploy/smolvla_deploy.sh
