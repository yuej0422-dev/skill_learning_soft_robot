...
def main() -> None:
    ...
    if not args.skip_unittest:
        results.append(
            run_cmd(
                [py, "-m", "unittest", "discover", "-s", "validation/automated"],
                cwd=PROJECT_ROOT,
            )
        )
    results.append(
        run_cmd(
            [py, "soft_vla/scripts/real_robot/diagnostics/inspect_real_control_assets.py", "--episode-index", str(args.episode_index)],
            cwd=REPO_ROOT,
        )
    )
    results.append(
        run_cmd(
            [py, "soft_vla/scripts/real_robot/diagnostics/test_pressure_driver.py", "--packet-channels", "16", "--pressure", "0.1"],
            cwd=REPO_ROOT,
        )
    )
    results.append(
